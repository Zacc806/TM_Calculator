...
продакшн-домен **https://calculator.atamuragroup.kz**. Битрикс24-портал — `amanat.bitrix24.kz`.
...
HTTPS: продакшн-домен **https://calculator.atamuragroup.kz** живой (A-запись
`calculator.atamuragroup.kz → 93.115.14.193`, валидный Let's Encrypt через Caddy).
Интерим **https://93-115-14-193.sslip.io** оставлен как fallback. Блоки — в `deploy/Caddyfile`;
после правки на сервере: `caddy reload --config /etc/caddy/Caddyfile`.
...
```html
<iframe id="atamura-calc" src="https://calculator.atamuragroup.kz/embed?price=25000000&zhk=atmosfera"
        style="width:100%;border:0" scrolling="no" title="Калькулятор платежа"></iframe>
<script>
  window.addEventListener("message", function (e) {
    if (e.origin !== "https://calculator.atamuragroup.kz") return;
...
## Заявки (лид-форма сайта)

Заявка всегда сохраняется на сервере в `LEADS_FILE` (`data/leads.jsonl`, одна строка = один
лид). Дополнительно — опциональные зеркала, включаются переменными окружения, без правки кода:
- **Telegram** — задать `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` (новый лид падает в чат/группу).
- **Битрикс24** — задать `BITRIX_WEBHOOK_URL` (scope `crm`) — зеркалит в `crm.lead.add`.

Просмотр: `cat /root/tm-calculator/data/leads.jsonl`.
